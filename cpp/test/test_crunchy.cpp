...
int main(int argc, char **argv) {
  ...
  describe("#compare", [](){
      ...
      it("Should confirm equality", [](){
          auto x = Crunchy.from_string("12345678901234567890");
          auto y = Crunchy.from_string("12345678901234567890");
      ...
      it("Should find first is greater than second", [](){
          auto x = Crunchy.from_string("12345678901234567890");
          auto y = Crunchy.from_string("1234567890");
          assert.isTrue(x.gt(y));
          // this.done();
          });

      it("Should find first is less than second", [](){
          auto x = Crunchy.from_string("1234567890");
          auto y = Crunchy.from_string("12345678901234567890");
  ...
  it("Should add positive to negative number expecting positive result", [](){
      auto x = Crunchy.from_8bit([51, 254, 144, 207]);
      auto y = Crunchy.from_8bit([-20, 89, 145, 32]);
  ...
  describe("#add-from_string", [](){
      ...
      it("Should add numbers", [](){
          ...
          auto x = Crunchy.from_string("12345678911234567891");
          // console.log(new Date());
          auto y = Crunchy.from_string("12345678901234567890");
      ...
      it("Should add zero to number", [](){
          auto x = Crunchy.from_string("12345678901234567890");
          auto y = Crunchy.from_string("0");
          assert.equal(x.add(y).toString(), "12345678901234567890");
          // this.done();
          });

      it("Should add number to zero", [](){
          auto x = Crunchy.from_string("0");
          auto y = Crunchy.from_string("12345678901234567890");
          assert.equal(x.add(y).toString(), "12345678901234567890");
          // this.done();
          });
  ...
  describe("#rightShift", [](){
      ...
      it("Should right shift a number to zero", [](){
          auto x = Crunchy.from_14bit([167837953]);
