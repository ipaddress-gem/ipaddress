...
describe('Ipv4Test', () => {
    ...
    function setup() : IPv4Test {
        ...
        ipv4t.valid_ipv4.insert("9.9/17",
                                IPv4Prefix {
                                    ip: String.from_str("9.0.0.9"),
                                    prefix: 17,
                                });
        ipv4t.valid_ipv4.insert("100.1.100",
                                IPv4Prefix {
                                    ip: String.from_str("100.1.0.100"),
                                    prefix: 32,
                                });
        ipv4t.valid_ipv4.insert("0.0.0.0/0",
                                IPv4Prefix {
                                    ip: String.from_str("0.0.0.0"),
                                    prefix: 0,
                                });
        ipv4t.valid_ipv4.insert("10.0.0.0",
                                IPv4Prefix {
                                    ip: String.from_str("10.0.0.0"),
                                    prefix: 32,
                                });
        ipv4t.valid_ipv4.insert("10.0.0.1",
                                IPv4Prefix {
                                    ip: String.from_str("10.0.0.1"),
                                    prefix: 32,
                                });
    ...
    it("test_method_to_u32", () => {
        for (addr, int) in setup().decimal_values {
            let ip = IPAddress.parse(addr);
            assert_eq!(ip.host_address.to_u32(), int);
        }
    });
    ...
    it("test_method_size", () => {
        let ip = IPAddress.parse("10.0.0.1/29");
        assert_eq!(ip.size(), BigUint.from_str("8"));
    }
    );
    ...
    it("test_method_octet", () => {
        assert_eq!(*setup().ip.parts().get(0), 172);
        assert_eq!(*setup().ip.parts().get(1), 16);
        assert_eq!(*setup().ip.parts().get(2), 10);
        assert_eq!(*setup().ip.parts().get(3), 1);
    }
    );
    it("test_method_a", () => {
    ...
    it("test_method_dns_rev_domains", () => {
        assert_eq!(IPAddress.parse("173.17.5.1/23").dns_rev_domains(),
                   ["4.17.173.in-addr.arpa", "5.17.173.in-addr.arpa"]);
        assert_eq!(IPAddress.parse("173.17.1.1/15").dns_rev_domains(),
                   ["16.173.in-addr.arpa", "17.173.in-addr.arpa"]);
        assert_eq!(IPAddress.parse("173.17.1.1/7").dns_rev_domains(),
                   ["172.in-addr.arpa", "173.in-addr.arpa"]);
        assert_eq!(IPAddress.parse("173.17.1.1/29").dns_rev_domains(),
                   [
        ...
        assert_eq!(IPAddress.parse("174.17.1.1/24").dns_rev_domains(),
                   ["1.17.174.in-addr.arpa"]);
        assert_eq!(IPAddress.parse("175.17.1.1/16").dns_rev_domains(),
                   ["17.175.in-addr.arpa"]);
        assert_eq!(IPAddress.parse("176.17.1.1/8").dns_rev_domains(),
                   ["176.in-addr.arpa"]);
        assert_eq!(IPAddress.parse("177.17.1.1/0").dns_rev_domains(),
                   ["in-addr.arpa"]);
        assert_eq!(IPAddress.parse("178.17.1.1/32").dns_rev_domains(),
    ...
    it("test_method_minus", () => {
        ...
        assert_eq!(9, ip2.sub(ip1).to_u32());
        assert_eq!(9, ip1.sub(ip2).to_u32());
    }
    );
    it("test_method_plus", () => {
    ...
    it("test_method_split", () => {
        ...
        assert_eq!(setup().ip.split(1), [setup().ip.network()]);

        assert_eq!(IPAddress.to_string_vec(setup().network.split(8)),
                   ["172.16.10.0/27",
                    "172.16.10.32/27",
                    "172.16.10.64/27",
                    "172.16.10.96/27",
        ...
        assert_eq!(IPAddress.to_string_vec(setup().network.split(7)),
                   ["172.16.10.0/27",
                    "172.16.10.32/27",
                    "172.16.10.64/27",
                    "172.16.10.96/27",
        ...
        assert_eq!(IPAddress.to_string_vec(setup().network.split(6)),
                   ["172.16.10.0/27",
                    "172.16.10.32/27",
                    "172.16.10.64/27",
                    "172.16.10.96/27",
                    "172.16.10.128/26",
                    "172.16.10.192/26"]);
        assert_eq!(IPAddress.to_string_vec(setup().network.split(5)),
                   ["172.16.10.0/27",
                    "172.16.10.32/27",
                    "172.16.10.64/27",
                    "172.16.10.96/27",
                    "172.16.10.128/25"]);
        assert_eq!(IPAddress.to_string_vec(setup().network.split(4)),
                   ["172.16.10.0/26", "172.16.10.64/26", "172.16.10.128/26", "172.16.10.192/26"]);
        assert_eq!(IPAddress.to_string_vec(setup().network.split(3)),
                   ["172.16.10.0/26", "172.16.10.64/26", "172.16.10.128/25"]);
        assert_eq!(IPAddress.to_string_vec(setup().network.split(2)),
                   ["172.16.10.0/25", "172.16.10.128/25"]);
        assert_eq!(IPAddress.to_string_vec(setup().network.split(1)),
                   ["172.16.10.0/24"]);
    }
    );
    it("test_method_subnet", () => {
        ...
        assert_eq!(IPAddress.to_string_vec(setup().network.subnet(26)),
                                 ["172.16.10.0/26",
                                  "172.16.10.64/26",
                                  "172.16.10.128/26",
                                  "172.16.10.192/26"]);
        assert_eq!(IPAddress.to_string_vec(setup().network.subnet(25)),
                   ["172.16.10.0/25", "172.16.10.128/25"]);
        assert_eq!(IPAddress.to_string_vec(setup().network.subnet(24)),
                   ["172.16.10.0/24"]);
    }
    );
