    ...
    impl IPv4Test {
        ...
        pub fn test_initialize() {
            ...
            for i in setup.valid_ipv4 {
                let ip = IPAddress::parse(i).unwrap();
...
#[test]
        ...
        pub fn test_method_each_host() {
            ...
            let expected = vec!["10.0.0.1", "10.0.0.2", "10.0.0.3", "10.0.0.4", "10.0.0.5",
                                "10.0.0.6"];
        ...
        pub fn test_method_each() {
            ...
            let expected = vec!["10.0.0.0", "10.0.0.1", "10.0.0.2", "10.0.0.3", "10.0.0.4",
        ...
        pub fn test_method_include() {
            ...
            assert_eq!(false, ip.includes(IPAddress::parse("172.16.0.48").unwrap()));
            ...
            assert_eq!(false, ip.includes(IPAddress::parse("172.16.0.0/16").unwrap()));
            ...
            assert_eq!(false, ip.includes(IPAddress::parse("13.16.0.0/32").unwrap()));
        }
        #[test]
        ...
        pub fn test_method_private() {
            assert_eq!(true, IPAddress::parse("192.168.10.50/24").unwrap().is_private());
            assert_eq!(true, IPAddress::parse("192.168.10.50/16").unwrap().is_private());
            assert_eq!(true, IPAddress::parse("172.16.77.40/24").unwrap().is_private());
            assert_eq!(true, IPAddress::parse("172.16.10.50/14").unwrap().is_private());
            assert_eq!(true, IPAddress::parse("10.10.10.10/10").unwrap().is_private());
            assert_eq!(true, IPAddress::parse("10.0.0.0/8").unwrap().is_private());
            assert_eq!(false, IPAddress::parse("192.168.10.50/12").unwrap().is_private());
            assert_eq!(false, IPAddress::parse("3.3.3.3").unwrap().is_private());
            assert_eq!(false, IPAddress::parse("10.0.0.0/7").unwrap().is_private());
            assert_eq!(false, IPAddress::parse("172.32.0.0/12").unwrap().is_private());
            assert_eq!(false, IPAddress::parse("172.16.0.0/11").unwrap().is_private());
            assert_eq!(false, IPAddress::parse("192.0.0.2/24").unwrap().is_private());
        }
        #[test]
        ...
        pub fn test_method_rev_domains() {
            assert_eq!(["4.17.173.in-addr.arpa", "5.17.173.in-addr.arpa"],
                       IPAddress::parse("173.17.5.1/23").unwrap().rev_domains());
            assert_eq!(["16.173.in-addr.arpa", "17.173.in-addr.arpa"],
                       IPAddress::parse("173.17.1.1/15").unwrap().rev_domains());
            assert_eq!(["172.in-addr.arpa", "173.in-addr.arpa"],
                       IPAddress::parse("173.17.1.1/7").unwrap().rev_domains());
            assert_eq!(["1.17.173.in-addr.arpa"],
                       IPAddress::parse("173.17.1.1/29").unwrap().rev_domains());
            assert_eq!(["1.17.174.in-addr.arpa"],
                       IPAddress::parse("174.17.1.1/24").unwrap().rev_domains());
            assert_eq!(["17.175.in-addr.arpa"],
                       IPAddress::parse("175.17.1.1/16").unwrap().rev_domains());
            assert_eq!(["176.in-addr.arpa"],
                       IPAddress::parse("176.17.1.1/8").unwrap().rev_domains());
            assert_eq!(["0.in-addr.arpa"],
                       IPAddress::parse("177.17.1.1/0").unwrap().rev_domains());
            assert_eq!(["1.17.178.in-addr.arpa"],
                       IPAddress::parse("178.17.1.1/32").unwrap().rev_domains());
        }
        #[test]
        pub fn test_method_compare() {
            ...
            arr = vec!["10.0.0.0/8", "10.0.0.0/16", "10.0.0.0/24"];
            assert_eq!(arr, [ip1, ip2, ip3].sort.iter().iter().map(|s| s.to_string()));
        }
        #[test]
        ...
        pub fn test_method_plus() {
            ...
            assert_eq!(["172.16.10.0/23"], (ip1 + ip2).iter().map(|i| i.to_string()));

            ip2 = IPAddress::parse("172.16.12.2/24").unwrap();
            assert_eq!([ip1.network.to_string(), ip2.network.to_string()],
                       (ip1 + ip2).iter().map(|i| i.to_string()));
            ...
            ip2 = IPAddress::parse("10.0.2.0/24").unwrap();
            assert_eq!(["10.0.0.0/23", "10.0.2.0/24"],
                       (ip2 + ip1).iter().map(|i| i.to_string()));

            ip1 = IPAddress::parse("10.0.0.0/16").unwrap();
            ip2 = IPAddress::parse("10.0.2.0/24").unwrap();
            assert_eq!(["10.0.0.0/16"], (ip1 + ip2).iter().map(|i| i.to_string()));

            ip1 = IPAddress::parse("10.0.0.0/23").unwrap();
            ip2 = IPAddress::parse("10.1.0.0/24").unwrap();
            assert_eq!(["10.0.0.0/23", "10.1.0.0/24"],
                       (ip1 + ip2).iter().map(|i| i.to_string()));
        }
        #[test]
        ...
        pub fn test_method_split() {
            ...
            let mut arr = vec!["172.16.10.0/27",
                           "172.16.10.32/27",
                           "172.16.10.64/27",
                           "172.16.10.96/27",
                           "172.16.10.128/27",
                           ...
                           "172.16.10.224/27"];
            assert_eq!(arr,
                       setup().network.split(8).unwrap().iter().map(|s| s.to_string()));

            arr = vec!["172.16.10.0/27",
                   "172.16.10.32/27",
                   "172.16.10.64/27",
                   "172.16.10.96/27",
                   "172.16.10.128/27",
                   ...
                   "172.16.10.192/26"];
            assert_eq!(arr,
                       setup().network.split(7).unwrap().iter().map(|s| s.to_string()));
            ...
            arr = vec!["172.16.10.0/27",
                   ...
                   "172.16.10.192/26"];
            assert_eq!(arr,
                       setup().network.split(6).unwrap().iter().map(|s| s.to_string()));
            arr = vec!["172.16.10.0/27",
                   "172.16.10.32/27",
                   "172.16.10.64/27",
                   "172.16.10.96/27",
                   "172.16.10.128/25"];
            assert_eq!(arr,
                       setup().network.split(5).unwrap().iter().map(|s| s.to_string()));
            arr = vec!["172.16.10.0/26", "172.16.10.64/26", "172.16.10.128/26", "172.16.10.192/26"];
            assert_eq!(arr,
                       setup().network.split(4).unwrap().iter().map(|s| s.to_string()));
            arr = vec!["172.16.10.0/26", "172.16.10.64/26", "172.16.10.128/25"];
            assert_eq!(arr,
                       setup().network.split(3).unwrap().iter().map(|s| s.to_string()));
            arr = vec!["172.16.10.0/25", "172.16.10.128/25"];
            assert_eq!(arr,
                       setup().network.split(2).unwrap().iter().map(|s| s.to_string()));
            arr = vec!["172.16.10.0/24"];
            assert_eq!(arr,
                       setup().network.split(1).unwrap().iter().map(|s| s.to_string()));
        }
        #[test]
        pub fn test_method_subnet() {
            ...
            let mut arr =
                vec!["172.16.10.0/26", "172.16.10.64/26", "172.16.10.128/26", "172.16.10.192/26"];
            assert_eq!(arr,
                       setup().network.subnet(26).unwrap().iter().map(|s| s.to_string()));
            arr = vec!["172.16.10.0/25", "172.16.10.128/25"];
            assert_eq!(arr,
                       setup().network.subnet(25).unwrap().iter().map(|s| s.to_string()));
            arr = vec!["172.16.10.0/24"];
            assert_eq!(arr,
                       setup().network.subnet(24).unwrap().iter().map(|s| s.to_string()));
        }
        #[test]
        ...
        pub fn test_classmethod_summarize() {
            ...
            assert_eq!(["172.16.10.0/23"],
                       IPAddress::summarize(ip1, ip2).iter().map(|i| i.to_string()));

            ip1 = IPAddress::parse("10.0.0.1/24").unwrap();
            ip2 = IPAddress::parse("10.0.1.1/24").unwrap();
            let mut ip3 = IPAddress::parse("10.0.2.1/24").unwrap();
            let mut ip4 = IPAddress::parse("10.0.3.1/24").unwrap();
            assert_eq!(["10.0.0.0/22"],
                       IPAddress::summarize(ip1, ip2, ip3, ip4).iter().map(|i| i.to_string()));
            assert_eq!(["10.0.0.0/22"],
                       IPAddress::summarize(ip4, ip3, ip2, ip1).iter().map(|i| i.to_string()));
            ...
            let mut result = vec!["10.0.1.0/24", "10.0.2.0/23", "10.0.4.0/24"];
            assert_eq!(result,
                       IPAddress::summarize(ip1, ip2, ip3, ip4).iter().map(|i| i.to_string()));
            assert_eq!(result,
                       IPAddress::summarize(ip4, ip3, ip2, ip1).iter().map(|i| i.to_string()));
            ...
            result = vec!["10.0.1.0/24", "10.10.2.0/24", "172.16.0.0/23"];
            assert_eq!(result,
                       IPAddress::summarize(ip1, ip2, ip3, ip4).map(|i| i.to_string()));

            let mut ips = vec![IPAddress::parse("10.0.0.12/30").unwrap(), IPAddress::parse("10.0.100.0/24").unwrap()];
            result = vec!["10.0.0.12/30", "10.0.100.0/24"];
            assert_eq!(result, IPAddress::summarize(ips).iter().map(|i| i.to_string()));

            ips = vec![IPAddress::parse("172.16.0.0/31").unwrap(), IPAddress::parse("10.10.2.1/32").unwrap()];
            result = vec!["10.10.2.1/32", "172.16.0.0/31"];
            assert_eq!(result, IPAddress::summarize(ips).iter().map(|i| i.to_string()));

            ips = vec![IPAddress::parse("172.16.0.0/32").unwrap(), IPAddress::parse("10.10.2.1/32").unwrap()];
            result = vec!["10.10.2.1/32", "172.16.0.0/32"];
            assert_eq!(result, IPAddress::summarize(ips).iter().map(|i| i.to_string()));
        }
