...
impl IPv4 {
    ...
    pub fn new(str: String) {
      ...
      if (netmask) { //  netmask is defined
        ...
        if (netmask =~ /^\d{1,2}$/) { //  netmask in cidr format
          ...
          if (IPAddress::valid_ipv4_netmask?(netmask)) {  //  netmask in IP format
    ...
    //
    pub fn to_ipv6() {
      return format!("{x04}:{x04}", (self.to_u32 >> 16) & 0xffff, self.to_u32 & 0xffff)
    }
    ...
    pub fn self.extract(str) {
      let re = Regexp::new(r"((25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)\.){3}(25[0-5]|2[0-4]\d|1\d\d|[1-9]\d|\d)")
...
impl Sub for IPv4 {
    ...
    //      // => 256
    //
    fn sub(&self, other: IPv4) -> u32 {
      return (self.to_u32 - oth.to_u32).abs()
    }
}
...
impl Add for IPv4 {
    ...
    //
    fn add(&self, other: IPv4) -> u32 {
      return IPv4::summarize(vec![self,oth]);
    }
}
