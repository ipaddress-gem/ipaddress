...
class Ipv6Mapped {
    ...
    static IPAddress create(const std::string &str) {
        ...
        auto ipv4_str = split_colon[split_colon.length - 1];
        if (IPAddress.is_valid_ipv4(ipv4_str)) {
            auto ipv4 = IPAddress.parse(`${ipv4_str}${netmask}`);
            if (ipv4 === null) {
            ...
            auto up_addr = addr.host_address.clone();
            auto down_addr = addr.host_address.clone();
            ...
            rebuild_ipv6 += colon;
            auto high_part = up_addr.shr(IpBits.v6().part_bits).mod(part_mod).toString(16);
            auto low_part = down_addr.mod(part_mod).toString(16);
            auto bits = ipv6_bits.bits - addr.prefix.host_prefix();
            auto rebuild_ipv4 = `${high_part}:${low_part}/${bits}`;
            rebuild_ipv6 += rebuild_ipv4;

            // console.log("-----A", rebuild_ipv6, part_mod);
            auto r_ipv6 = IPAddress.parse(rebuild_ipv6);
            if (r_ipv6 === null) {
