...
mod tests {
    ...
    impl IPAddressTest {
        ...
        pub fn test_summarize() {
            ...
            let nets = netstr.map(|i| IPAddress::parse(i));

            assert_eq!(vec![], IPAddress::summarize([]));
            assert_eq!(vec!["10.1.0.0/24"],
                       IPAddress::summarize(["10.1.0.4/24"]).iter().map(|i| i.to_string()));
            assert_eq!(vec!["2000:1::/32"],
                       IPAddress::summarize(["2000:1::4711/32"]).iter().map(|i| i.to_string()));
...

            {
                ...
                assert_eq!(vec!["2000:1::/32",
                                ...
                                "2000:8::/32",
                                "3000:1::/32"],
                           IPAddress::summarize(networks).iter().map(|i| i.to_string()));
            }
            ...
            {
                let networks = vec!["10.0.0.0/23", "10.0.2.0/24"];
                assert_eq!(["10.0.0.0/23", "10.0.2.0/24"],
                           IPAddress::summarize(networks).iter().map(|i| i.to_string()));
            }
            {
                let networks = vec!["10.0.0.0/24", "10.0.1.0/24", "10.0.2.0/23"];
                assert_eq!(["10.0.0.0/22"],
                           IPAddress::summarize(networks).iter().map(|i| i.to_string()));
            }


            assert_eq!(vec!["10.0.0.0/16"],
                       IPAddress::summarize(&vec!["10.0.0.0/16", "10.0.2.0/24"])
                           .iter()
                           .map(|i| i.to_string()));


            for i in 0..100 {
                assert_eq!(vec!["1.0.0.0/8",
                                "2.0.0.0/7",
                                "4.0.0.0/6",
                                "8.0.0.0/7",
                                "11.0.0.0/8",
            ...
            assert_eq!(&vec!["10.0.0.0/23"],
                       IPAddress::summarize(vec![a1, a2]).iter().map(|i| i.to_string()));
